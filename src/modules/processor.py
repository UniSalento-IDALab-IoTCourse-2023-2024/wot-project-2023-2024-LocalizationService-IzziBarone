...
class PreClustering:
    ...
    def get_clusters(self, data=True):
        """
        Restituisce i dati clusterizzati o il numero di cluster
        :param data: Se True restituisce i dati clusterizzati
        :return: Dati clusterizzati o numero di cluster
        """
        clusters = {}
        if self.data is not None and data:
    ...
    def predict(self, new_data, results=False):
        ...
        if results:  # Return cluster and data
            return cluster[0], self.data[self.data['cluster'] == cluster[0]]
    ...
    def mle_predict(self, current_rssi, results=False):
        ...
        if (self.data is not None and 'cluster' not in self.data.columns) and (self._histograms is None):
...
class IndoorPositioning:
    ...
    def _predict(self, current_rssi, clean=True):
        import pandas as pd
        """
        ...
        if self.data and self._category not in self.data.columns:
        ...
        if isinstance(self._scaler, MinMaxScaler) and clean:
    ...
    def predict(self, data,clean=True):
        """
        Effettua la predizione del modello KNN
        :param data: Lista contenente i valori RSS correnti per ciascun AP
        :return: L'RP stimato
        """
        return self._predict(data,clean=clean)
